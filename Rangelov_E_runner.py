...
from Rangelov_E_player import BestAI, RandomAI
...
# variables
player_types = {0: "Player", 1: "Random", 2: "AI"}
...
def find_moves(my_board, my_color):
    ...
    for c in range(len(my_board)):
        for r in range(0, 6):
            if r == 5 and my_board[c][r] == ".":
                moves_found.add(c * y_max + r)
                break
            if my_board[c][r] != ".":
                moves_found.add((c) * y_max + (r-1))
                break
    return moves_found
...
def check_win(my_board, turn, x, y):
    if turn == black:
        color = "O"
    else:
        color = "X"
    for incr in directions:
        ...
        x_pos = x
        y_pos = y
        while 0 <= x_pos < x_max and 0 <= y_pos < y_max and board[x_pos][y_pos] == color:
            count += 1
            x_pos += incr[0]
            y_pos += incr[1]
            if count == 4:
            ...
            if x_pos < 0 or x_pos >= x_max or y_pos < 0 or y_pos >= y_max:
                x_pos = x
                y_pos = y
                if incr == directions[0]:
                    incr = directions[7]
                elif incr == directions[1]:
                    incr = directions[6]
                elif incr == directions[2]:
                    incr = directions[5]
                elif incr == directions[3]:
                    incr = directions[4]
                elif incr == directions[4]:
                    incr = directions[3]
                elif incr == directions[5]:
                    incr = directions[2]
                elif incr == directions[6]:
                    incr = directions[1]
                elif incr == directions[7]:
                    incr = directions[0]
                x_pos += incr[0]
                y_pos += incr[1]
                while 0 <= x_pos < x_max and 0 <= y_pos < y_max and board[x_pos][y_pos] == color:
...
def next_turn(x_pos, y_pos):
    ...
    if check_win(board, turn, x_pos, y_pos):
        print_board(board)
...
def init(choice_menu, e1, e2, v1, v2):
    ...
    if players[black] == "Random":
        players[black] = RandomAI()
    elif players[black] == "AI":
        players[black] = BestAI()
    if players[white] == "Random":
        players[white] = RandomAI()
    elif players[white] == "AI":
        players[white] = BestAI()
    choice_menu.destroy()
    root = tk.Tk()
...
def menu():
    ...
    tk.Radiobutton(text="Random", font=("Arial", 20), bg=white, fg=black, anchor=tk.W, variable=v2, value=1).grid(row=2,
                                                                                                                  column=1,
                                                                                                                  sticky=tk.W + tk.E + tk.N + tk.S)
    tk.Radiobutton(text="AI", font=("Arial", 20), bg=black, fg=grey, anchor=tk.W, variable=v1, value=2).grid(row=3,
                                                                                                                 column=0,
                                                                                                                 sticky=tk.W + tk.E + tk.N + tk.S)
    tk.Radiobutton(text="AI", font=("Arial", 20), bg=white, fg=black, anchor=tk.W, variable=v2, value=2).grid(row=3,
                                                                                                                  column=1,
                                                                                                                  sticky=tk.W + tk.E + tk.N + tk.S)
